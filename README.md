...
- Copy the files in a folder named "scripts" and a folder "synology_dhcp_dns_autoupdate" inside the "scripts" folder in the home folder of the admin account:
...
1. Install two scripts into the "admin" account.  These scripts should be owned by root and executable:
    ...
    ```
    DiskStation> ls -l /var/services/homes/admin/scripts/synology_dhcp_dns_autoupdate/*sh
    -rwxr-xr-x    1 root     root          7798 May  1 15:07 /var/services/homes/admin/scripts/synology_dhcp_dns_autoupdate/diskstation_dns_modify.sh
    -rwxr-xr-x    1 root     root           283 Nov 21  2014 /var/services/homes/admin/scripts/synology_dhcp_dns_autoupdate/admin/poll-dhcp-changes.sh
    ```
...
2. Install the settings file to the admin directory:
    ...
    ```
    DiskStation> ls -l /var/services/homes/admin/scripts/synology_dhcp_dns_autoupdate/settings                              
    -rw-r--r--    1 root     root           109 Sep 24  2015 /var/services/homes/admin/scripts/synology_dhcp_dns_autoupdate/settings
    ```
...
```sh
        /usr/local/etc/rc.d/S99pollDHCP.sh start >>/var/services/homes/admin/scripts/synology_dhcp_dns_autoupdate/startS99pollDHCP.log
```
...
Each time this script detects that that there is a DHCP change, DNS will be updated.  It may take up to 10 seconds for DNS to be updated after a new DHCP reservation.  A log file of this process is kept at `/var/services/homes/admin/scripts/synology_dhcp_dns_autoupdate/logs/dhcp-dns.log`.  
